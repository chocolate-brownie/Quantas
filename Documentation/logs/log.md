...
- Switched `Packet` serialization from `nlohmann::json` `to_json`/`from_json` to Boost serialization (per the professor's preference). Used the intrusive split `save()`/`load()` member pattern with `BOOST_SERIALIZATION_SPLIT_MEMBER()` since `nlohmann::json` is not natively Boost-serializable — `_body` is dumped to `std::string` on save and parsed back on load. `_delay` and `_round` remain unserialized (Abstract-only concepts).
- Implemented `unicastTo()` end-to-end in `NetworkInterfaceConcreteMQ`: build Packet → serialize via `binary_oarchive` into `std::stringstream` → extract bytes with `ss.str()` → open destination's queue with `open_only` → guard `bytes.size()` against `mq.get_max_msg_size()` → `send()`. Replaced the hardcoded `MAX_MSG_SIZE` check with a runtime query so the function doesn't depend on `ProcessCoordinatorMQ.hpp` and stays correct if the queue is resized later.
- Implemented `receive()`: `try_receive()` loop draining the inbox each round → wrap `recvd_size` bytes in a `std::stringstream` → deserialize into a `Packet` via `binary_iarchive` → push into `_inStream` under `_inStream_mtx`. Loop exits when `try_receive()` returns `false` (queue empty), matching the round-synchronous model of the Abstract version.
- Key intuition locked in: `_myInbox` is a persistent handle to my own queue (`peer_<myId>`), opened once in `configure()` and reused. The local `mq` in `unicastTo()` is a temporary handle to someone else's queue (`peer_<dest>`) — same OS queue, different handle, like file descriptors pointing at the same file.
- Critical detail: in `receive()`, the deserialization stringstream must be built from `std::string(buffer.data(), recvd_size)` — not `buffer.size()` — since the buffer is always `max_msg_size` bytes but only `recvd_size` were actually written by the sender. Using `buffer.size()` would feed garbage bytes into the archive.
- Tomorrow (Wed 29/04): supervisor checkpoint. Open questions: max expected message size for PBFT (1024 may be too small), and whether `_delay`/`_round` should be serialized for fault-injection compatibility later.
