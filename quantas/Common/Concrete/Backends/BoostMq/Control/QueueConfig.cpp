...
BoostMqQueueConfig parseBoostMqConfig(const nlohmann::json& experiment, int initialPeers) {
    ...
    if (!experiment.contains("boostMq"))
        return config;

    const auto& boostMq = experiment["boostMq"];

    if (!boostMq.is_object())
        throw std::runtime_error("boostMq must be a JSON object");
    ...
    if (boostMq.contains("readyTimeoutMs")) {
        const auto& value = boostMq["readyTimeoutMs"];

        if (!value.is_number_unsigned() || value.get<std::size_t>() == 0)
            throw std::runtime_error("boostMq.readyTimeoutMs must be a positive whole number");
...
void validateBoostMqAssignmentPayloads(const std::vector<PeerAssignment>& assignments,
    ...
    for (const PeerAssignment& assignment : assignments) {
        ...
        if (payloadSize <= queueConfig.maxMessageSizeBytes)
            continue;
...
void preflightBoostMqQueues(const BoostMqQueueConfig& queueConfig, std::size_t experimentIndex) {
    ...
    const std::size_t largestCapacity =
        std::max(queueConfig.controlQueueCapacity, queueConfig.dataQueueCapacity);

    if (largestCapacity >
        std::numeric_limits<std::size_t>::max() / queueConfig.maxMessageSizeBytes) {
        throw std::runtime_error("BoostMQ preflight failed for experiment " +
                                 std::to_string(experimentIndex) +
                                 ": requested queue size exceeds the supported size range");
    }

    const std::string queueSuffix =
        std::to_string(::getpid()) + "_" + std::to_string(experimentIndex);
    ...
    try {
        boost::interprocess::message_queue temporaryControlQueue(
            boost::interprocess::create_only, controlQueueName.c_str(),
            queueConfig.controlQueueCapacity, queueConfig.maxMessageSizeBytes);
        ...
        boost::interprocess::message_queue temporaryDataQueue(
            boost::interprocess::create_only, dataQueueName.c_str(), queueConfig.dataQueueCapacity,
            queueConfig.maxMessageSizeBytes);
        QUANTAS_LOG_INFO("preflight") << "temporary data queue created successfully";
    } catch (const boost::interprocess::interprocess_exception& ex) {
        removeTemporaryQueues();
        throw std::runtime_error("BoostMQ preflight failed for experiment " +
                                 std::to_string(experimentIndex) + ": " + ex.what());
    }
