...
class PeerRegistry {
  public:
    ...
    static Peer *makePeer(const std::string &type, interfaceId pubId = 0) {
        ...
        if (inst->registry.bucket_count() == 0)
            throw std::runtime_error(
                "unordered_map has zero buckets in makePeer() in file Peer.hpp"
            );
    ...
    static bool registerPeerType(
        const std::string &name, std::function<Peer *(interfaceId)> factory
    ) {
        PeerRegistry *inst = instance();
        bool result =
            inst->registry.insert(std::make_pair(name, factory)).second;
        if (!result) {
            std::cout << "Peer of type:" << name << " already registered."
                      << std::endl;
        }
        return result;
    }
  ...
  private:
    ...
    std::unordered_map<std::string, std::function<Peer *(interfaceId)>>
        registry;
};
...
class Peer {
  public:
    inline Peer() {}
    inline Peer(NetworkInterface *networkInterface)
    ...
    /* Called once after constructing all peers (subclass can parse 'parameters'
     * as needed) */
    virtual void
    initParameters(const std::vector<Peer *> &peers, json parameters) {}
    ...
    // Called after performComputation in each round (subclass can override to
    // collect metrics, etc.)
    virtual void endOfRound(std::vector<Peer *> &peers) {}

    bool isCrashed() {
        return (_crashRecoveryRound > RoundManager::currentRound());
    }
    void setCrashRecoveryRound(size_t crashRecoveryRound) {
        _crashRecoveryRound = crashRecoveryRound;
    }
    ...
    std::set<interfaceId> neighbors() const {
        return _networkInterface->neighbors();
    };
    ...
    void removeNeighbor(interfaceId nbr) {
        _networkInterface->removeNeighbor(nbr);
    };

    // Send messages to to others using these
    virtual void unicastTo(json msg, const interfaceId &dest) {
