...
class PeerRegistry {
  public:
    ...
    static bool
    registerPeerType(const std::string &name, std::function<Peer *(interfaceId)> factory) {
        PeerRegistry *inst = instance();
...
class Peer {
  public:
    ...
    ////////////////// Network Interface direct acccss ////////////////////

    // safe way to replace a peer's NetworkInterfafce after construction.
    void setNetworkInterface(NetworkInterface *networkInterface) {
        if (networkInterface == nullptr)
            throw std::invalid_argument("networkInterface cannot be null");
